...
async function load(url) {
    try {
        ...
        // 2. Direct String Extraction (Bypasses JSON.parse issues)
        // This looks for all URLs inside the .show() function calls you provided
        const episodes = [];
        const seenLinks = new Set();

        // We target the scripts you sent: pw.show, fsst.show, go.show, ollhd.show
        // The regex finds all https links inside those brackets
        const linkRegex = /https?:\/\/kinoger\.[^'"]+/g;
        const allLinks = html.match(linkRegex) || [];

        // Kinoger usually repeats the same episode list for different qualities/tabs.
        // We group them by their position in the list (Episode 1, Episode 2, etc.)
        // According to your HTML, there are 8 episodes.
        
        // We need to find the longest list to ensure we get all episodes
        const scriptBlocks = html.match(/\.show\(\d+,\s*\[\[([\s\S]*?)\]\]\)/g) || [];
        
        if (scriptBlocks.length > 0) {
            // Pick the first block (e.g., pw.show) and extract its specific links
            const firstBlockLinks = scriptBlocks[0].match(/https?:\/\/[^'"]+/g) || [];
            
            firstBlockLinks.forEach((link, index) => {
                ...
                if (!seenLinks.has(cleanLink)) {
                    episodes.push({
                        ...
                        season: 1,
                        episode: index + 1,
                        // Save the link for the next step (loadLinks)
                        data: JSON.stringify({ url: cleanLink })
                    });
        ...
        // 3. Determine if it's a Movie (DLE '0.2' flag)
